...
const slashDefs = [
  ...
  new SlashCommandBuilder().setName('rain').setDescription('[ADMIN] Rain coins — react to enter, 2 min timer').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addIntegerOption(o=>o.setName('amount').setDescription('Total coins to rain').setRequired(true).setMinValue(10)),
  new SlashCommandBuilder().setName('shop').setDescription('View all items and prices'),
  new SlashCommandBuilder().setName('redeem').setDescription('Buy an item from the shop').addStringOption(o=>o.setName('item').setDescription('Item to buy').setRequired(true).addChoices(
  ...
  new SlashCommandBuilder().setName('claim').setDescription('Submit a delivery claim for an item').addStringOption(o=>o.setName('id').setDescription('Claim ID, e.g. C1').setRequired(true)),
  new SlashCommandBuilder().setName('use-code').setDescription('Redeem a code for coins').addStringOption(o=>o.setName('code').setDescription('The code to redeem').setRequired(true)),
  ...
  new SlashCommandBuilder().setName('claimed').setDescription('[ADMIN] Mark a claim as fulfilled').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addStringOption(o=>o.setName('id').setDescription('Claim ID').setRequired(true)),
  new SlashCommandBuilder().setName('deny-claim').setDescription('[ADMIN] Deny a claim and refund to inventory').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addStringOption(o=>o.setName('id').setDescription('Claim ID').setRequired(true)),
  new SlashCommandBuilder().setName('update-robux').setDescription('[ADMIN] Update Robux stock').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addIntegerOption(o=>o.setName('amount').setDescription('New amount').setRequired(true).setMinValue(0)),
  new SlashCommandBuilder().setName('update-etfb').setDescription('[ADMIN] Update ETFB stock').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addStringOption(o=>o.setName('type').setDescription('Which item').setRequired(true).addChoices({name:'Divines',value:'divines'},{name:'Celestials',value:'celestials'})).addIntegerOption(o=>o.setName('amount').setDescription('New amount').setRequired(true).setMinValue(0)),
  new SlashCommandBuilder().setName('give').setDescription('[ADMIN] Give coins to a user').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addUserOption(o=>o.setName('user').setDescription('Target').setRequired(true)).addIntegerOption(o=>o.setName('amount').setDescription('Amount').setRequired(true).setMinValue(1)),
  new SlashCommandBuilder().setName('take').setDescription('[ADMIN] Take coins from a user').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addUserOption(o=>o.setName('user').setDescription('Target').setRequired(true)).addIntegerOption(o=>o.setName('amount').setDescription('Amount').setRequired(true).setMinValue(1)),
  new SlashCommandBuilder().setName('remove-inv').setDescription('[ADMIN] Remove an item from a user inventory').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addUserOption(o=>o.setName('user').setDescription('Target user').setRequired(true)).addStringOption(o=>o.setName('claim_id').setDescription('Claim ID to remove').setRequired(true)),
  new SlashCommandBuilder().setName('check-inventory').setDescription('[ADMIN] View any user inventory').setDefaultMemberPermissions(PermissionFlagsBits.Administrator).addUserOption(o=>o.setName('user').setDescription('Target user').setRequired(true)),
].map(c => c.toJSON());

// ══════════════════════════════════════════
//  REGISTER COMMANDS — runs once on startup
//  Retries every 10s until it succeeds
// ══════════════════════════════════════════
async function registerCommands(appId) {
  for (let attempt = 1; attempt <= 10; attempt++) {
    try {
      console.log(`Command registration attempt ${attempt}...`);
      ...
      if (res.ok) { console.log(`✅ Registered ${json.length} slash commands`); return; }
      console.error(`Attempt ${attempt} failed (${res.status}):`, JSON.stringify(json));
    } catch (e) {
      console.error(`Attempt ${attempt} error:`, e.message);
    }
    await new Promise(r => setTimeout(r, 10_000)); // wait 10s then retry
  }
  console.error('❌ Command registration failed after 10 attempts');
}
...
client.once('ready', async () => {
  ...
  // Register commands in background — retries until success
...
async function cmdInventory(reply, userId, username) {
  ...
  return reply({ embeds: [new EmbedBuilder().setTitle(`🎒 ${username}'s Inventory`).setColor(0x9B59B6)
    .setDescription(list).setFooter({ text: `${inv.length} item(s) · /claim <id> to submit` })] });
}
...
async function cmdHelp(reply) {
  return reply({ embeds: [new EmbedBuilder().setTitle('📖 Help').setColor(0x5865F2).addFields(
    { name:'💰 Economy', value:`/balance — check your ${COIN_EMOJI}\n/daily — 10–15 ${COIN_EMOJI} every 24h\n/leaderboard — top 10\n💬 Every message = 1 ${COIN_EMOJI}`, inline:false },
    { name:'🛒 Shop',    value:`/shop — view items & prices\n/redeem — buy an item\n/inventory — view your items\n/claim <id> — submit a delivery claim`, inline:false },
    { name:'🎟️ Codes',  value:`/use-code <code>`, inline:false }
  )] });
}
...
async function cmdRain(msgOrInteraction, guild, senderId, senderName, amount) {
  ...
  const rainEmbed = new EmbedBuilder().setColor(0x3498DB).setTitle('🌧️ Coin Rain — React to Enter!')
  ...
  setTimeout(async () => {
    try {
      ...
      await rainMsg.reply({ embeds:[new EmbedBuilder().setColor(0x57F287).setTitle('🌧️ Rain Finished!')
...
client.on('interactionCreate', async interaction => {
  ...
  try {
    ...
    if (cmd==='claims') {
      await interaction.deferReply({ flags:MessageFlags.Ephemeral });
      const pending = (Array.isArray(await getClaims())?await getClaims():[]).filter(c=>c.status==='pending');
    ...
    if (cmd==='claimed') {
      ...
      if (arr[idx].status==='denied')    return interaction.editReply({ embeds:[errEmbed(`Was denied.`)] });
      const claim = arr[idx];
      ...
      let dmSent=false;
      try { const t=await client.users.fetch(claim.userId); await t.send({ embeds:[new EmbedBuilder().setColor(0x57F287).setTitle('🎉 Reward Delivered!').setDescription(`✅ ${dmText}`).addFields({name:'Claim ID',value:`\`${claimId}\``,inline:true},{name:'Item',value:claim.itemName,inline:true},{name:'Roblox',value:claim.robloxUsername,inline:true})] }); dmSent=true; } catch {}
      ...
      try {
        ...
        if (vch) {
          ...
          const vt = setTimeout(async () => {
            ...
            try { const u=await client.users.fetch(claim.userId); await u.send({ embeds:[new EmbedBuilder().setColor(0xFEE75C).setTitle("⭐ Don't forget to vouch!").setDescription(`Head to <#${VOUCH_CHANNEL_ID}> and type:\n\`Vouch @${me.username} <reason>\`\n\nIt only takes a second! 🙏`)] }); } catch {}
            try { const ach=await client.channels.fetch(ALERT_CHANNEL_ID); if(ach) await ach.send({ embeds:[new EmbedBuilder().setColor(0xED4245).setTitle('⚠️ Vouch Not Received').setDescription(`<@${claim.userId}> hasn't vouched after receiving **${claim.itemName}** (\`${claimId}\`).`)] }); } catch {}
          }, 10*60*1000);
    ...
    if (cmd==='deny-claim') {
      ...
      const claim=arr[idx]; arr[idx].status='denied'; arr[idx].deniedAt=Date.now(); arr[idx].deniedBy=me.username;
      await saveClaims(arr);
      ...
      await saveStore(store); await updateStockEmbed(client);
      let dmSent=false;
      try { const t=await client.users.fetch(claim.userId); await t.send({ embeds:[new EmbedBuilder().setColor(0xED4245).setTitle('❌ Claim Denied').setDescription(`Your claim \`${claimId}\` for **${claim.itemName}** was denied.\n\nThe item has been returned to your inventory.\nUse \`/claim ${claimId}\` to re-submit.`)] }); dmSent=true; } catch {}
      return interaction.editReply({ embeds:[new EmbedBuilder().setColor(0xED4245).setTitle('❌ Claim Denied').addFields({name:'Claim',value:`\`${claimId}\``,inline:true},{name:'User',value:`<@${claim.userId}>`,inline:true},{name:'Item',value:claim.itemName,inline:true},{name:'Refunded',value:'✅ Inventory',inline:true},{name:'Stock',value:'✅ Restored',inline:true},{name:'DM',value:dmSent?'✅ Sent':'❌ DMs off',inline:true})] });
    }
